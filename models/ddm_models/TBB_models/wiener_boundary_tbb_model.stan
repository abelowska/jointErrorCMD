...
model {
    ...
    alpha_ne_sd ~ normal(0,.5);
    alpha_ne_pre_acc_sd ~ normal(0,.5);
    delta_sd ~ gamma(1,1);
...
generated quantities { 
    ...
    vector[n_participants] participants_alpha_ern;
    vector[n_participants] participants_alpha_crn;
    //  vector[n_participants] participants_delta_ern;
    // vector[n_participants] participants_delta_crn;


    // group-level parameters
    alpha_ern = alpha_ne + alpha_ne_pre_acc*(-1);
    alpha_crn = alpha_ne + alpha_ne_pre_acc;
    // delta_ern = delta_ne + delta_ne_pre_acc*(-1);
    // delta_crn = delta_ne + delta_ne_pre_acc;

    // individual-level parameters
    for (p in 1:n_participants) {
        participants_alpha_ern[p] = participants_alpha_ne[p] + participants_alpha_ne_pre_acc[p]*(-1);
        participants_alpha_crn[p] = participants_alpha_ne[p] + participants_alpha_ne_pre_acc[p];
