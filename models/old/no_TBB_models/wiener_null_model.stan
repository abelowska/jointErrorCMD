wfunctions {
    ...
    */
    ...
    real participant_level_diffusion_lpdf(
        ...
        for (t in 1:n_trials) {
            if (abs(y[t]) - ndt > 0) {
                participant_level_likelihood[t] = diffusion_lpdf(y[t] | boundary  + boundary_cond*condition[t], ndt, bias, drift + drift_cond*condition[t]);
            } else {
                participant_level_likelihood[t] = diffusion_lpdf(ndt | boundary  + boundary_cond*condition[t], ndt, bias, drift + drift_cond*condition[t]);
            }
        }
...
model {
    ...
    for (p in 1:n_participants) {
        ...
        target += participant_level_diffusion_lpdf( y[participants_trials_slices[p][1]:participants_trials_slices[p][2]] | participants_alpha[p], participants_alpha_cond[p], participants_ter[p], 0.5, participants_delta[p], participants_delta_cond[p], condition[participants_trials_slices[p][1]:participants_trials_slices[p][2]], (participants_trials_slices[p][2] - participants_trials_slices[p][1] + 1));         
    }
}
