    ...
    */
    ...
    real partial_diffusion_lpdf(
        ...
        for (t in 1:n_trials) {
            if (abs(y_slice[t]) - (ndt[participant[global_index]] + ndt_ne*pre_ne[global_index] + ndt_cond*condition[global_index]) > 0) {
                ...
                partial_sum_level_likelihood[t] = diffusion_lpdf(ndt[participant[global_index]] + ndt_ne*pre_ne[global_index] + ndt_cond*condition[global_index] | boundary[participant[global_index]]  + boundary_cond[participant[global_index]]*condition[global_index], ndt[participant[global_index]] + ndt_ne*pre_ne[global_index] + ndt_cond*condition[global_index], bias, drift[participant[global_index]] + drift_cond[participant[global_index]]*condition[global_index]);
            }
    ...
    real log_likelihood_diffusion_lpdf(
        ...
        if (abs(y) - ndt > 0) {
            ...
            return diffusion_lpdf(ndt | boundary + boundary_cond*condition, ndt + ndt_ne*pre_ne + ndt_cond*condition, bias, drift + drift_cond*condition);
        }
    }
}
...
model {
    ...
    ter_cond ~ normal(0,0.01) T[-0.03, 0.03];
