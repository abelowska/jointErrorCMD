        ...
        for (t in 1:n_trials) {
            if (abs(y_slice[t]) - ndt[participant[global_index]] > 0) {
                partial_sum_level_likelihood[t] = diffusion_lpdf(y_slice[t] | boundary[participant[global_index]]  + boundary_cond[participant[global_index]]*condition[global_index] + boundary_ne[participant[global_index]]*pre_ne[global_index] + 
                    boundary_pre_acc*pre_acc[global_index] + boundary_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index] + boundary_ne_cond*pre_ne[global_index]*condition[global_index] + boundary_pre_acc_cond*pre_acc[global_index]*condition[global_index] + 
                    ...
                    drift[participant[global_index]] + drift_cond[participant[global_index]]*condition[global_index] + drift_ne[participant[global_index]]*pre_ne[global_index] + drift_pre_acc*pre_acc[global_index] + drift_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index] + 
...
model {
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_diffusion_lpdf, y, grainsize, participants_alpha, participants_alpha_cond, participants_alpha_ne, alpha_pre_acc, alpha_ne_pre_acc, alpha_ne_cond, alpha_pre_acc_cond, alpha_ne_pre_acc_cond, participants_ter, 0.5, 
        participants_delta, participants_delta_cond, participants_delta_ne, delta_pre_acc, delta_ne_pre_acc, delta_ne_cond, delta_pre_acc_cond, delta_ne_pre_acc_cond, condition, pre_ne, pre_acc, participant);         
}
