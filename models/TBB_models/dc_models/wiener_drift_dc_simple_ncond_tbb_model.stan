functions {
    ...
    */
    ...
    real log_likelihood_diffusion_lpdf(
        ...
        return diffusion_lpdf(y | varsigma, ndt, bias, drift + drift_ne*pre_ne + drift_pre_acc*pre_acc + drift_ne_pre_acc*pre_ne*pre_acc);
    }
}
...
model {
    ...
    for (p in 1:n_participants) {
        ...
        // Participant-level diffusion coefficient
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_diffusion_lpdf, y, grainsize, participants_varsigma, participants_ter, 0.5, 
   ...
   // Wiener likelihood
    for (i in 1:N) {
        // Log density for DDM process
         log_lik[i] = log_likelihood_diffusion_lpdf(y[i] | participants_varsigma[participant[i]], participants_ter[participant[i]], 0.5, participants_delta[participant[i]], participants_delta_ne[participant[i]], delta_pre_acc, participants_delta_ne_pre_acc[participant[i]], pre_ne[i], pre_acc[i]);
    }
}
