functions {
    ...
    */
    ...
    real partial_diffusion_lpdf(
        ...
        for (t in 1:n_trials) {
            if (abs(y_slice[t]) - ndt[participant[global_index]] > 0) {
                partial_sum_level_likelihood[t] = diffusion_lpdf(y_slice[t] | boundary[participant[global_index]]  + boundary_cond[participant[global_index]]*condition[global_index] + boundary_ne[participant[global_index]]*pre_ne[global_index] + boundary_pre_acc*pre_acc[global_index] + boundary_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index] + boundary_ne_cond*pre_ne[global_index]*condition[global_index] + boundary_pre_acc_cond*pre_acc[global_index]*condition[global_index] + boundary_ne_pre_acc_cond*pre_ne[global_index]*pre_acc[global_index]*condition[global_index], ndt[participant[global_index]], bias, drift[participant[global_index]] + drift_cond[participant[global_index]]*condition[global_index]);
            } else {
                partial_sum_level_likelihood[t] = diffusion_lpdf(ndt[participant[start+t-1]] | boundary[participant[start+t-1]]  + boundary_cond[participant[start+t-1]]*condition[start+t-1] + boundary_ne[participant[start+t-1]]*pre_ne[start+t-1] + boundary_pre_acc*pre_acc[start+t-1] + boundary_ne_pre_acc*pre_ne[start+t-1]*pre_acc[start+t-1] + boundary_ne_cond*pre_ne[start+t-1]*condition[start+t-1] + boundary_pre_acc_cond*pre_acc[start+t-1]*condition[start+t-1] + boundary_ne_pre_acc_cond*pre_ne[start+t-1]*pre_acc[start+t-1]*condition[start+t-1], ndt[participant[start+t-1]], bias, drift[participant[start+t-1]] + drift_cond[participant[start+t-1]]*condition[start+t-1]);
            }
    ...
    real log_likelihood_diffusion_lpdf(
        ...
        if (abs(y) - ndt > 0) {
            return diffusion_lpdf(y | boundary  + boundary_cond*condition + boundary_ne*pre_ne + boundary_pre_acc*pre_acc + boundary_ne_pre_acc*pre_ne*pre_acc + boundary_ne_cond*pre_ne*condition + boundary_pre_acc_cond*pre_acc*condition + boundary_ne_pre_acc_cond*pre_ne*pre_acc*condition, ndt, bias, drift + drift_cond*condition);
        } else {
            return diffusion_lpdf(ndt | boundary  + boundary_cond*condition + boundary_ne*pre_ne + boundary_pre_acc*pre_acc + boundary_ne_pre_acc*pre_ne*pre_acc + boundary_ne_cond*pre_ne*condition + boundary_pre_acc_cond*pre_acc*condition + boundary_ne_pre_acc_cond*pre_ne*pre_acc*condition, ndt, bias, drift + drift_cond*condition);
        }
    }
}
...
model {
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_diffusion_lpdf, y, grainsize, participants_alpha, participants_alpha_cond, participants_alpha_ne, alpha_pre_acc, alpha_ne_pre_acc, alpha_ne_cond, alpha_pre_acc_cond, alpha_ne_pre_acc_cond, participants_ter, 0.5, participants_delta, participants_delta_cond, condition, pre_ne, pre_acc, participant);         
}
