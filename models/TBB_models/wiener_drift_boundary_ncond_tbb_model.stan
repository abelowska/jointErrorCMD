functions {
    ...
    */
    ...
    real partial_diffusion_lpdf(
        ...
        for (t in 1:n_trials) {
            if (abs(y_slice[t]) - ndt[participant[global_index]] > 0) {
                partial_sum_level_likelihood[t] = diffusion_lpdf(y_slice[t] | boundary[participant[global_index]] + boundary_ne[participant[global_index]]*pre_ne[global_index] + 
                    boundary_pre_acc*pre_acc[global_index] + boundary_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index], ndt[participant[global_index]], bias, 
                ...
                partial_sum_level_likelihood[t] = diffusion_lpdf(ndt[participant[global_index]] | boundary[participant[global_index]] + boundary_ne[participant[global_index]]*pre_ne[global_index] + 
                    boundary_pre_acc*pre_acc[global_index] + boundary_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index], ndt[participant[global_index]], bias, 
                    drift[participant[global_index]] + drift_ne[participant[global_index]]*pre_ne[global_index] + drift_pre_acc*pre_acc[global_index] + drift_ne_pre_acc*pre_ne[global_index]*pre_acc[global_index]);
            }
    ...
    real log_likelihood_diffusion_lpdf(
        ...
        if (abs(y) - ndt > 0) {
            return diffusion_lpdf(y | boundary + boundary_ne*pre_ne + boundary_pre_acc*pre_acc + boundary_ne_pre_acc*pre_ne*pre_acc, ndt, bias, drift + drift_ne*pre_ne + drift_pre_acc*pre_acc + drift_ne_pre_acc*pre_ne*pre_acc);
        } else {
            return diffusion_lpdf(ndt | boundary + boundary_ne*pre_ne + boundary_pre_acc*pre_acc + boundary_ne_pre_acc*pre_ne*pre_acc, ndt, bias, drift + drift_ne*pre_ne + drift_pre_acc*pre_acc + drift_ne_pre_acc*pre_ne*pre_acc);
        }
    }
}
...
model {
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_diffusion_lpdf, y, grainsize, participants_alpha, participants_alpha_ne, alpha_pre_acc, alpha_ne_pre_acc, participants_ter, 0.5, 
   ...
   // Wiener likelihood
    for (i in 1:N) {
        // Log density for DDM process
         log_lik[i] = log_likelihood_diffusion_lpdf(y[i] | participants_alpha[participant[i]], participants_alpha_ne[participant[i]], alpha_pre_acc, alpha_ne_pre_acc, participants_ter[participant[i]], 0.5, participants_delta[participant[i]], participants_delta_ne[participant[i]], delta_pre_acc, delta_ne_pre_acc, pre_ne[i], pre_acc[i]);
    }
}
