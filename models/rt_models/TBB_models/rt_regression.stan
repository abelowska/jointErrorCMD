functions{
    real partial_normal_lpdf(
        ...
        for (t in 1:n_trials) {
            partial_sum_level_likelihood[t] = normal_lpdf(y_slice[t] | participants_mu[participant[global_index]] + participants_cond[participant[global_index]]*condition[global_index] + 
                participants_ne[participant[global_index]]*pre_ne[global_index] + ne_cond*pre_ne[global_index]*condition[global_index] + participants_acc[participant[global_index]]*pre_acc[global_index] + 
                acc_cond*pre_acc[global_index]*condition[global_index] + participants_ne_acc[participant[global_index]]*pre_ne[global_index]*pre_acc[global_index] + ne_acc_cond*pre_ne[global_index]*pre_acc[global_index]*condition[global_index], sigma);
...
model {
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_diffusion_lpdf, rt, grainsize, participants_mu, participants_cond, participants_ne, ne_cond, participants_acc, acc_cond, ne_acc, ne_acc_cond, sigma, participant);        
}
...
generated quantities { 
   vector[N] log_lik; 
    ...
    for (p in 1:n_participants) {
        participants_ern[p] = participants_ne[p] + participants_ne_pre_acc[p]*(-1);
        participants_crn[p] = participants_ne[p] + participants_ne_pre_acc[p];
    }
