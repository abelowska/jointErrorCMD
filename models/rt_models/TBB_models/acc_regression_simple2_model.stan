functions{
    real partial_bernoulli_logit_lpmf(
        ...
        for (t in 1:n_trials) {
            partial_sum_level_likelihood[t] = bernoulli_logit_lpmf(
                y_slice[t] | mu[participant[global_index]] + 
                cond[participant[global_index]]*condition[global_index] + 
                ne[participant[global_index]]*pre_ne[global_index] + 
                ne_cond*pre_ne[global_index]*condition[global_index] + 
                acc*pre_acc[global_index] + 
                acc_cond*pre_acc[global_index]*condition[global_index] + 
                ne_acc[participant[global_index]]*pre_ne[global_index]*pre_acc[global_index] + 
                ne_acc_cond*pre_ne[global_index]*pre_acc[global_index]*condition[global_index]
                );
...
model {
    ...
    int grainsize = 100;     
    target += reduce_sum(partial_bernoulli_logit_lpmf, y_acc, grainsize, participants_mu, participants_cond, participants_ne, 
        acc,
        ne_cond, 
        acc_cond, 
        participants_ne_acc, 
