functions {
    ...
    */
    ...
    real participant_level_diffusion_lpdf(
        ...
        for (t in 1:n_trials) {
            if (abs(y[t]) - ndt > 0) {
                participant_level_likelihood[t] = diffusion_lpdf(y[t] | boundary + boundary_cond*condition[t], ndt, bias, drift + drift_cond*condition[t] + drift_ne*pre_ne[t] + drift_ne_cond*pre_ne[t]*condition[t] + drift_pre_acc*pre_acc[t] + drift_pre_acc_cond*pre_acc[t]*condition[t] + drift_ne_pre_acc*pre_ne[t]*pre_acc[t] + drift_ne_pre_acc_cond*pre_ne[t]*pre_acc[t]*condition[t]);
            } else {
